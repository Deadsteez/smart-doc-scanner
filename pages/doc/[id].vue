...
<AppNavbar />
 ...
 <NuxtLink to="/" class="text-blue-400 hover:underline mb-6 inline-block text-sm"> ← Back to Dashboard
 </NuxtLink>
...
<img :src="doc.image" class="rounded-lg max-w-full shadow-md" alt="Scanned document"/>
    ...
    <div class="grid grid-cols-2 gap-4 text-sm text-gray-700 dark:text-gray-300">
     <div>
       <span class="block text-gray-500 text-xs mb-0.5">Vendor</span>{{ extracted.vendor || '—' }}
     </div>
   <div>
      <span class="block text-gray-500 text-xs mb-0.5">Date</span>{{ extracted.date || '—' }}
     </div>

     <div>
      <span class="block text-gray-500 text-xs mb-0.5">Total</span>{{ extracted.total || '—' }}
     </div>

  <div> <span class="block text-gray-500 text-xs mb-0.5">Receipt #</span>{{ extracted.receiptNumber || '—' }}  </div>

  <div> <span class="block text-gray-500 text-xs mb-0.5">Tax</span> {{ extracted.tax || '—' }} </div>
  
  <div> <span class="block text-gray-500 text-xs mb-0.5">Payment Method</span> {{ extracted.paymentMethod || '—' }} </div>
  </div>
 </div>
...
<div class="space-y-2">
 <div  v-for="(item, index) in extracted.items"  :key="index" class="flex justify-between items-center text-sm border-b border-gray-200 dark:border-gray-800 pb-2 last:border-0">
