  ...
  <div class="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
    ...
    <div v-if="isProcessing" class="p-6">
      ...
      <div class="w-full bg-gray-800 rounded-full h-2 overflow-hidden">
        <div class="h-full bg-blue-500 transition-all duration-300"
          :style="{ width: `${(progress?.progress || 0) * 100}%` }"></div>
      </div>
    ...
    <div v-if="hasPages && !isProcessing" class="p-4">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mb-4 max-h-96 overflow-y-auto">
        <div v-for="page in pdfPages" :key="page.pageNumber"
          class="relative border rounded-lg overflow-hidden cursor-pointer transition-all"
          :class="selectedPages.has(page.pageNumber) ? 'border-blue-500 ring-2 ring-blue-500/50' : 'border-gray-700 hover:border-gray-600'"
          ...
          <div class="absolute top-1 left-1 bg-black/70 text-white text-xs px-1.5 py-0.5 rounded">{{ page.pageNumber }}
          </div>

          <div v-if="selectedPages.has(page.pageNumber)"
      ...
      <div class="flex gap-3">
        <button @click="proceedWithSelected" :disabled="!canProceed"
