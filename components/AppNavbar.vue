      ...
      <div :class="[
        ...
        <div class="flex items-center gap-8">
          <NuxtLink to="/" class="flex items-center gap-2.5 group">
            ...
            <span class="text-text-primary font-semibold tracking-tight text-sm">SmartScan</span>
          </NuxtLink>

          <div class="flex items-center gap-1">
            <NuxtLink to="/dashboard" class="px-3 py-1.5 rounded-lg text-sm font-medium transition-colors" :class="$route.path === '/dashboard'
              ? 'text-text-primary bg-bg-tertiary/60'
              : 'text-text-muted hover:text-text-secondary hover:bg-bg-tertiary/30'">
              Dashboard
            </NuxtLink>
            <NuxtLink to="/scan" class="px-3 py-1.5 rounded-lg text-sm font-medium transition-colors" :class="$route.path === '/scan'
              ? 'text-text-primary bg-bg-tertiary/60'
              : 'text-text-muted hover:text-text-secondary hover:bg-bg-tertiary/30'">
              Scan
            </NuxtLink>
            <NuxtLink to="/workspace" class="px-3 py-1.5 rounded-lg text-sm font-medium transition-colors" :class="$route.path.startsWith('/workspace')
              ? 'text-text-primary bg-bg-tertiary/60'
              : 'text-text-muted hover:text-text-secondary hover:bg-bg-tertiary/30'">
              Workspace
            </NuxtLink>
        ...
        <div class="flex items-center gap-3">
          ...
          <template v-if="user">
            <div class="relative">
              <button @click="toggleProfileMenu" aria-label="Open profile menu"
                class="flex items-center gap-2 p-1 rounded-lg hover:bg-bg-tertiary/40 transition-all duration-200">
                <div class="w-8 h-8 rounded-full bg-accent-primary/20 flex items-center justify-center text-accent-primary text-xs font-bold tracking-wide select-none ring-1 ring-accent-primary/30 hover:bg-accent-primary/30 transition-colors">
                  {{ userInitials }}
                </div>
              </button>
              ...
              <Transition
                ...
                <div v-if="showProfileMenu" ref="profileMenuRef"
                  ...
                  <div class="px-4 py-3">
                    ...
                    <p class="text-xs text-text-muted mt-1">Signed in</p>
                  </div>
                  <div class="p-2">
          ...
          <template v-else>
            <NuxtLink to="/login" class="text-sm text-text-muted hover:text-text-primary transition-colors px-3 py-1.5">
              Login
            </NuxtLink>
  ...
  <nav class="sticky top-0 z-40 md:hidden glass-panel border-b border-slate-1/30">
    <div class="flex items-center justify-between px-4 py-3">
      ...
      <div class="flex items-center gap-2">
        <button @click="toggleTheme" class="p-2 rounded-lg text-text-muted hover:text-text-secondary transition-colors">
        ...
        <template v-if="user">
          <NuxtLink to="/profile" aria-label="Go to profile"
