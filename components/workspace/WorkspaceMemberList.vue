  ...
  <div id="workspace-member-list" class="space-y-2">
    ...
    >
      ...
      <div class="flex items-center gap-3 min-w-0">
        <div class="w-9 h-9 rounded-full bg-accent-primary/15 flex items-center justify-center text-accent-primary font-bold text-sm select-none flex-shrink-0">
          {{ (member.email ?? '?').slice(0, 2).toUpperCase() }}
        </div>
        <div class="min-w-0">
          <p class="text-sm font-medium text-text-primary truncate">{{ member.email ?? member.user_id.slice(0, 12) + '…' }}</p>
      ...
      <div class="flex items-center gap-2 flex-shrink-0">
        <span
          class="text-xs font-semibold px-2.5 py-1 rounded-lg uppercase tracking-wide"
        ...
        <button
          v-if="isAdmin && member.user_id !== workspaceStore.currentUserId"
        ...
        >
          <svg v-if="removingId === member.id" class="w-4 h-4 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 12a8 8 0 018-8"/></svg>
