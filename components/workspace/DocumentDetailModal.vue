  ...
  <Teleport to="body">
    <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0"
      ...
      <div class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4"
        @click.self="emit('close')" id="document-detail-modal">
        <Transition enter-active-class="transition duration-200 ease-out"
          enter-from-class="opacity-0 scale-95 translate-y-2" enter-to-class="opacity-100 scale-100 translate-y-0">
          <div
            class="w-full max-w-2xl bg-bg-elevated rounded-2xl border border-slate-1/40 shadow-elevated overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-slate-1/20">
              ...
              <button id="modal-close-btn" @click="emit('close')"
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              <div v-if="documentImage"
                class="rounded-xl overflow-hidden border border-slate-1/20 bg-bg-primary max-h-56 flex items-center justify-center">
              ...
              <div v-if="approval" class="bg-bg-secondary rounded-xl p-4 border border-slate-1/20 space-y-4">
                <div>
                  ...
                  <p class="text-sm text-text-primary flex items-center gap-2">
                    <span
                      class="w-6 h-6 rounded-full bg-accent-primary/15 flex items-center justify-center text-accent-primary text-xs font-bold">
                ...
                <div v-if="approval.notes">
                  <p
                    class="text-[10px] text-text-muted uppercase tracking-wider mb-2 font-medium border-t border-slate-1/20 pt-3">
                    Reviewer Notes</p>
              ...
              <div v-if="actionError"
                class="bg-error/10 border border-error/20 rounded-xl px-4 py-3 text-sm text-error">
                {{ actionError }}
              </div>
            </div>
            ...
            <div class="border-t border-slate-1/20 px-6 py-4 space-y-3">
              ...
              <template v-if="isAdmin && approval?.status === 'pending'">
                <textarea v-model="reviewNotes" placeholder="Add reviewer notes (optional)…" rows="2"
                ...
                <div class="flex gap-3">
                  <button id="approve-btn" @click="handleApprove" :disabled="actionLoading"
                    ...
                    <svg v-if="actionLoading" class="w-4 h-4 animate-spin" fill="none" stroke="currentColor"
                      viewBox="0 0 24 24">
                  ...
                  <button id="reject-btn" @click="handleReject" :disabled="actionLoading"
              ...
              <template v-else-if="approval && approval.status === 'rejected' && isSubmitter(approval.submitted_by)">
                <button id="resubmit-btn" @click="handleResubmit" :disabled="actionLoading"
              ...
              <template v-else-if="!approval && !isAdmin">
                <button id="submit-approval-btn" @click="handleSubmitForApproval" :disabled="actionLoading"
              ...
              <div class="flex gap-3">
                ...
                <button v-if="localDoc?.userId === workspaceStore.currentUserId" @click="handleDeleteDocument"
