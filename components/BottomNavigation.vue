    ...
    <div class="glass-panel border-t border-slate-1/50 shadow-elevated">
      <div class="flex items-center justify-around px-2 py-1.5 safe-area-pb">
        <template v-for="item in navItems" :key="item.label">
          ...
          <NuxtLink v-if="item.isCta" :to="item.to" class="relative -mt-6 flex flex-col items-center">
            <div
            ...
            <span class="text-[10px] mt-1 font-medium text-accent-primary">{{ item.label }}</span>
          </NuxtLink>
          ...
          <NuxtLink v-else :to="item.to"
            ...
            <svg v-else-if="item.icon === 'docs'" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
              stroke-width="1.8">
              <path stroke-linecap="round" stroke-linejoin="round"
                d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
            <svg v-else-if="item.icon === 'workspace'" class="w-5 h-5" fill="none" stroke="currentColor"
              viewBox="0 0 24 24" stroke-width="1.8">
              <path stroke-linecap="round" stroke-linejoin="round"
                d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
            </svg>
            <svg v-else-if="item.icon === 'profile'" class="w-5 h-5" fill="none" stroke="currentColor"
              viewBox="0 0 24 24" stroke-width="1.8">
              <path stroke-linecap="round" stroke-linejoin="round"
